...
private fun CloudSetupContent(
    ...
    var lastTapMark by remember { mutableStateOf<kotlin.time.TimeSource.Monotonic.ValueTimeMark?>(null) }
    ...
    Column(
        ...
        Icon(
            ...
            modifier = Modifier
                ...
                .clickable {
                    ...
                    if (lastTapMark == null || (now - lastTapMark!!).inWholeSeconds > 3) {
                        tapCount = 1
                    } else {
                        tapCount++
                    }
        ...
        Text(
            text = stringResource(Res.string.cloud_setup_title),
            style = MaterialTheme.typography.headlineSmall,
        )

        Text(
            text = stringResource(Res.string.cloud_setup_subtitle),
        ...
        if (useDevKey) {
            AssistChip(
                onClick = {},
                label = { Text(stringResource(Res.string.cloud_setup_using_dev_key)) },
                leadingIcon = {
                    Icon(
                        imageVector = Icons.Default.Cloud,
                        contentDescription = null,
        ...
        OutlinedTextField(
            ...
            label = {
                Text(
                    stringResource(when (selectedProvider) {
                        "OPENAI" -> Res.string.cloud_setup_openai_key_label
                        "GEMINI" -> Res.string.cloud_setup_gemini_key_label
                        else -> Res.string.cloud_setup_api_key_label
                    }),
                )
            },
            placeholder = {
                Text(
                    stringResource(when (selectedProvider) {
                        "OPENAI" -> Res.string.cloud_setup_openai_placeholder
                        "GEMINI" -> Res.string.cloud_setup_gemini_placeholder
                        else -> Res.string.cloud_setup_key_placeholder
                    }),
                )
            },
            singleLine = true,
            ...
            trailingIcon = {
                IconButton(onClick = { passwordVisible = !passwordVisible }) {
                    Icon(
                        ...
                        contentDescription = stringResource(if (passwordVisible) Res.string.cloud_setup_hide_key else Res.string.cloud_setup_show_key),
                    )
                }
            },
            enabled = !useDevKey,
        ...
        // Model selector
        Text(
            text = stringResource(Res.string.cloud_setup_select_model),
        ...
        Button(
            ...
            Text(stringResource(Res.string.cloud_setup_save))
        }
    }
}
