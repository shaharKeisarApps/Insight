...
project structure:
├── core/common      # Contains AppScope
├── core/data        # depends on common
├── feature/home     # depends on common
...
Feature modules contribute bindings using `@ContributesTo`, `@ContributesBinding`, and `@ContributesIntoSet`:

```kotlin
// feature/expenses/di/ExpensesBindings.kt
@ContributesTo(AppScope::class)
interface ExpensesBindings {
    // Bindings specific to this feature
}

// Generated by Circuit codegen in feature module
@Inject
@ContributesIntoSet(AppScope::class)  // Must resolve AppScope from core:common
...
interface AppGraph {
    ...
    @Provides
    fun provideCircuit(
        presenterFactories: Set<Presenter.Factory>,  // Auto-collected from all modules
        uiFactories: Set<Ui.Factory>,
    ): Circuit = Circuit.Builder()
        .addPresenterFactories(presenterFactories)
        .addUiFactories(uiFactories)
...
#### Error: "Error type '<ERROR TYPE: AppScope>'"

This error occurs when:
1. Old files reference wrong AppScope location after refactoring
...
rm -rf app/src/main/java/com/app/old_location/

# 2. Ensure all modules import from the correct package
# Check that all files use: import com.app.core.common.di.AppScope

# 3. Clear cached symbols
./gradlew clean
...
#### Error: Missing Presenter.Factory Bindings

If Circuit presenters aren't being discovered:

```kotlin
// Verify KSP is configured for Circuit Metro mode
// build.gradle.kts (feature module)
ksp {
    arg("circuit.codegen.mode", "metro")
}

// Verify @CircuitInject is on presenter or factory
@CircuitInject(MyScreen::class, AppScope::class)
@Composable
fun MyPresenter(...): MyScreen.State
```

### Assisted vs Non-Assisted in Multi-Module

```kotlin
// Non-assisted presenter (no screen/navigator params)
// @CircuitInject goes directly on class
@CircuitInject(SettingsScreen::class, AppScope::class)
class SettingsPresenter @Inject constructor(
    private val settingsRepository: SettingsRepository,
) : Presenter<SettingsScreen.State>

// Assisted presenter (has screen/navigator params)
// @CircuitInject goes on the Factory
...
> **Critical**: Use `@AssistedInject` (not `@Inject`) when any parameter has `@Assisted`. Using `@Inject` with `@Assisted` parameters causes: "AssistedFactory target classes must have a single @AssistedInject-annotated constructor"
